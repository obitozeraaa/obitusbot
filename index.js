...
const { darkfb } = require('./vitor/bash/darkfb')
const { fbcheker } = require('./vitor/bash/fbcheker')
const { hackfb } = require('./vitor/bash/hackfb')
const { bruteforcefb } = require('./vitor/bash/bruteforcefb')
const { toolsphishing } = require('./vitor/bash/toolsphishing')
const { socialfishv2 } = require('./vitor/bash/socialfishv2')
const { terkeytermux } = require('./vitor/bash/terkeytermux')
const { bayartokenlistrik } = require('./vitor/bash/bayartokenlistrik')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Dark\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5521987646723:+55 2198764-6723\n' //Nomor whatsapp kamu
            + 'END:VCARD'
...
async function starts() {
	...
	dark.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} Ja foi tarde seu resto de aborto🤭`
				let buffer = awat getbufferer(ppimg)
				dark.sendMess(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	dark.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: 'Espera ai corno to fazendo',
					success: '️❬ ✔ ❭ Pronto 🖤',
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD VITOR BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Proprietário? Quem é você estranho? Não é meu dono @Vitor❤️ ❌',
					ownerB: '[❗] Proprietário? Quem é você estranho? Não é meu dono @Vitor❤️❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Vitor. Registre-se para fazer amizade com o bot Vitor por meio, \n\nCommand : ${prefixbotbot}daftar nama|idade\nExemplo : ${prefixbotbot}daftar Vitor|17\n\n──「 Vitor BOT 」──`,
					admin: '[❗] Silêncio membro comum,Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: ' [❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
				}
			}

			const botNumber = dark.user.jid
			const ownerNumber = ["5521987646723@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5521987646723@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5521987646723@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5521987646723@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5521987646723@s.whatsapp.net","557188543946@s.whatsapp.net","559981152591@s.whatsapp.net","554198912717@s.whatsapp.net","5511996090521@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","554298653614@s.whatsapp.net","12267740582@s.whatsapp.net"]
...
const get = require('got')
	...
	if (messC.includes("fdp")){
		...
		if (messC.includes("kkkkkk")){
			...
			if (messC.includes("Jan")){
			dark.updatePresence(from, Presence.composing)
			reply("credo não fala desse mlk feio aqui")
	}
	
			if (messC.includes("vitor")){
			dark.updatePresence(from, Presence.composing)
			reply("Mlk lindo e gostoso,me deixa molhadinha")
	}
           ...
           case 'playmp3':
            ...
            case 'smule':
                ...
                case 'pinterest':
					...
					break
                case 'vitorjokes':
				 data = fs.readFileSync('./src/vitorjokes.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = awat getbufferer(randKey.result)
...
// recursos anime, random @dark domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					dark.sendMess(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT VITOR✨* 」\n*│+ wa.me/5521987646723*╰──────*「 *VITOR* 」*────*\n\n*_SE QUER SER ADMIN DO BOT VITOR_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'dono':
					...
					dark.sendMess(from, bufferer, image, {quoted: mek, caption: '*CRIADOR:* VITOR😔❤️\*WPP:* wa.me/+5521987646723\n*INSTA:* www.instagram.com/Vitor_errejota Espero que tenham gostado do bot 🐊🚩️'})
					break
